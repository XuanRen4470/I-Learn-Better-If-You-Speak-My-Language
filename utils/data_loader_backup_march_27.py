...
from utils.function import extract_GSM8K_numerical_final_answer_using_regex, extract_MATH_numerical_final_answer_using_regex


def load_MATH(path, n_row, extract_numerical_final_answer = False, minimum_change_or_zero_shot = False, zeroshot = False):
    question_list = []
    answer_list = []
    data_list = []
...
def load_MATH_march_27(path, n_row, extract_numerical_final_answer = False, zeroshot = False):
    question_list = []
    answer_list = []
    data_list = []
    with open(path, 'r') as file:
...
FINAL ANSWER: """
                ...
                # print('-------------------------------')    
                # print(line['answer'])
                # print(str(evaluate_expression(num)))
                # print('-------------------------------')
                # print()
                answer_list.append(answer)
                data_list.append(line)
...
def load_GSM8K(path, n_row, extract_numerical_final_answer = False, zeroshot = False):
    question_list = []
    answer_list = []
    data_list = []
    with open(path, 'r') as file:
...
Please inference then provide the final answer (a number) at the end.
"""
            if zeroshot:
                temp['question'] += f"""
...
"""
                ...
                prompt += \
f"""
Please inference first, then provide the answer(1/2).
"""
...
"""
                ...
                prompt += \
f"""
Please inference first, then provide the answer (True/False).
"""
...
"""
                ...
                prompt += \
f"""
Please inference first, then provide the answer(A/B/C/D).
"""
...
def load_AGIEVAL(path, n_row, finetune = False):
    data_list = []
    with open(path, 'r') as file:
        data = json.load(file)
        for line in data:
...
Then provide the answer (Entailment/Neutral/Contradiction) at the end.
"""
            try:
                answer_item = line['answer']
            except:
...
def load_CODE(path, n_row, minimum_change = False):
    data_list = []
    with open(path, 'r') as file:
        data = json.load(file)
        for line in data:
...
def load_dataset(HOME_DIRECTORY, continual_learning, multi_task_learning, train_task_name, n_train, enable_learn_from_mistakes, enable_REST_em_initial_500, enable_REST_em_initial_100, enable_change_model_internal_preference, enable_step_by_step_gpt4, variation_suffix_code, variation_suffix, enable_dpo, task_sequence_list = ['all']):
    import random
    intermediate_dpo_finetune_file_path = ''
    intermediate_dpo_minimum_change_train_file_path = ''
    intermediate_dpo_gpt4_train_file_path = ''
    ...
    if not multi_task_or_continual_learning:
        if train_task_name.upper() =='GSM8K':
            ...
            train_data_list = load_GSM8K(train_path, n_train, extract_numerical_final_answer = True)
            ...
            if enable_learn_from_mistakes:
                ...
                with open(f'{HOME_DIRECTORY}/dataset/GSM8K/gsm8k_minimum_change_100_learn_from_mistakes_cleaned.json', 'r') as file:
                    train_data_list = json.load(file)
...
Incorrect Step: """
            ...
            train_data_list = load_MATH(train_path, n_train, extract_numerical_final_answer = True, minimum_change_or_zero_shot = False)
            # train_path = f'{HOME_DIRECTORY}/dataset/GSM8K/minimum_change_train_dec11.json'
            minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/MATH/minimum_change_train_number_theory.json'
            minimum_change_train_data_list = load_MATH(minimum_change_train_path, n_train, minimum_change_or_zero_shot = True)
            gpt4_generated_data_train_path = f'{HOME_DIRECTORY}/dataset/MATH/gpt4_generated_{train_task_name.lower()}_data_step_by_step_False_500.json'
            with open(gpt4_generated_data_train_path, 'r') as file:
                gpt4_generated_train_data_list = json.load(file)
    ...
    #        api_bank_minimum_change_100_feb8
            ...
            minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/api_bank_minimum_change_100_mar_17.json'
            ...
            if variation_suffix == '':
                minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/anli_minimum_change_100_march_9_300.json'
            elif variation_suffix == 'old':
                minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/anli_minimum_change_9999_march_9_old.json'
            ...
            minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/mnli_minimum_change.json'
            with open(minimum_change_train_path, 'r') as file:
                minimum_change_train_data_list_temp = json.load(file)       
            ...
            minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/snli_minimum_change_100.json'
            with open(minimum_change_train_path, 'r') as file:
                minimum_change_train_data_list_temp = json.load(file)       
            ...
            # minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/scitail_minimum_change_100.json'
            minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/scitail_minimum_change_100_step_2.json'
            ...
            gpt4_generated_data_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/gpt4_generated_{train_task_name.lower()}_data_step_by_step_False_500{variation_suffix_code}.json'
            with open(gpt4_generated_data_train_path, 'r') as file:
                gpt4_generated_train_data_list = json.load(file)
            gpt4_generated_train_data_list = gpt4_generated_train_data_list[:n_train]
            ...
            minimum_change_train_path = f'{HOME_DIRECTORY}/dataset/{train_task_name.upper()}/piqa_minimum_change_100.json'
        ...
        if train_task_name.lower() != 'code' and train_task_name.lower() != 'api_bank' and train_task_name.lower() != 'mbpp' and train_task_name.lower() != 'mnli' and train_task_name.lower() != 'snli' and train_task_name.lower() != 'scitail' and train_task_name.lower() != 'boolq' and train_task_name.lower() != 'piqa' and train_task_name.lower() != 'winogrande' and train_task_name.lower() != 'mmlu' and train_task_name.lower() != 'agieval' and not multi_task_or_continual_learning:
            intermediate_dpo_minimum_change_train_file_name = f'dpo_{train_task_name}_minimum_change_{n_train}'
            intermediate_dpo_minimum_change_train_file_path = f"{HOME_DIRECTORY}/alpaca_data/{intermediate_dpo_minimum_change_train_file_name}.json"
            with open(intermediate_dpo_minimum_change_train_file_path, 'w') as json_file:
                json.dump(dpo_mc_train_data_list, json_file, indent=4)
